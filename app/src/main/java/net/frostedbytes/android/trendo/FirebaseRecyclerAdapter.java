...
public abstract class FirebaseRecyclerAdapter<ViewHolder extends RecyclerView.ViewHolder, T> extends RecyclerView.Adapter<ViewHolder> {
  ...
  /**
   * @param query The Firebase location to watch for data changes. Can also be a slice of a location, using some combination of <code>limit()</code>,
   ...
   * @param query The Firebase location to watch for data changes. Can also be a slice of a location, using some combination of <code>limit()</code>,
   * <code>startAt()</code>, and <code>endAt()</code>.
   * @param items List of items that will load the adapter before starting the listener. Generally null or empty, but this can be useful when dealing
   * with a configuration change (e.g.: reloading the adapter after a device rotation). Be careful: keys must be coherent with this list.
   * @param keys List of keys of items that will load the adapter before starting the listener. Generally null or empty, but this can be useful when
   * dealing with a configuration change (e.g.: reloading the adapter after a device rotation). Be careful: items must be coherent with this list.
   */
  ...
  private ChildEventListener mListener = new ChildEventListener() {
    ...
    public void onCancelled(DatabaseError databaseError) {
      ...
      System.out.println("Listen was cancelled, no more updates will occur.");
    }
  };
